...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_data_split", type=str, default=None, help="Test data split"
    )
    parser.add_argument(
        "--output_file_path", type=str, default=None, help="Output file"
    )
    ...
    test_data = load_dataset(
        "TIGER-Lab/LongRAG", args.test_data_name, split=args.test_data_split
    )
    ...
    for item in tqdm(test_data, desc="Evaluating QA"):
        ...
        try:
            if args.test_data_name == "nq":
                long_ans, short_ans = llm_inference.predict_nq(
                    context, question, context_titles
                )
            elif args.test_data_name == "hotpot_qa":
                long_ans, short_ans = llm_inference.predict_hotpotqa(
                    context, question, context_titles
                )
        except:
            long_ans, short_ans = "", ""
        ...
        if args.test_data_name == "nq":
            ...
            is_retrieval = (item["sp"][0] in context_titles) and (
                item["sp"][1] in context_titles
            )
